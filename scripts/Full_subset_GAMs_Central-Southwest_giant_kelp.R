...
# 1. Load central-southwest sites ----
sites <- read.csv(paste(d.dir, "giant_kelp_site_region_metadata.csv", sep = '/')) %>%
  ...
  dplyr::filter(region == "central-southwest") %>%
  glimpse()
...
dat1 <- df.region %>%
  ...
  dplyr::select(
    ...
    'Min_Monthly_Anomaly_Upwelling_Temp', # Log
    'Days_17C', # Log
    'Days_18C', # Log
    'Days_19C', # Log
    'Days_20C', # Log
    ...
    'Max_Monthly_Anomaly_Nitrate', # log
  ...
  mutate(log_Max_Monthly_Anomaly_Upwelling_Temp = log(Max_Monthly_Anomaly_Upwelling_Temp + 1),
         log_Days_18C = log(Days_17C + 1),
         log_Days_18C = log(Days_18C + 1),
         log_Days_19C = log(Days_19C + 1),
         log_Days_20C = log(Days_20C + 1),
         log_Days_21C = log(Days_21C + 1)
         ) %>%
  dplyr::select(-c(Days_17C, Days_18C, Days_19C, Days_20C, Days_21C, Max_Monthly_Anomaly_Upwelling_Temp)) %>%
  # Nitrate transformations
  mutate(log_Days_2N = log(Days_2N + 1),
         log_Days_3N = log(Days_3N + 1),
         log_Days_4N = log(Days_4N + 1),
         log_Max_Monthly_Anomaly_Nitrate = log(Max_Monthly_Anomaly_Nitrate + 1)) %>%
  dplyr::select(-c(Days_2N, Days_3N, Days_4N, Max_Monthly_Anomaly_Nitrate)) %>%
  ...
  dplyr::select(-c(wh_mean, UBR_Mean, UBR_Max)) %>%
...
pred.vars <- names(train.gam)[8:37]
